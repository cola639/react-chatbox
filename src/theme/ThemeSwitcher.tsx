...
export function ThemeSwitcherProvider(props: ThemeSwitcherProviderProps) {
  ...
  const [realMode, setRealMode] = useState<RealThemeMode>(
    getThemeModeFromLocal(REAL_THEME_MODE, ThemeMode.Dark)
  )
  ...
  const theme = useMemo(
    () => createTheme(fetchThemeDesign(realMode, settings.fontSize)),
    [realMode, settings]
  )
  ...
  useLayoutEffect(() => {
    document
      .querySelector('html')
      ?.setAttribute('data-theme', realMode === ThemeMode.Dark ? 'dark' : 'light')
  }, [realMode])
